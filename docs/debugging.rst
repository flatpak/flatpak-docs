...
Before debugging, it is useful to install the debug packages used by the
application.

First, get the SDK used by the application::

  $ flatpak info --show-sdk $FLATPAK_ID
  org.freedesktop.Sdk/x86_64/23.08

Now install the SDK and the Debug package for that SDK::

  $ flatpak install org.freedesktop.Sdk/x86_64/23.08
  $ flatpak install org.freedesktop.Sdk.Debug/x86_64/23.08

Additionally install the Debug package for the application if available::

  $ flatpak install $FLATPAK_ID.Debug

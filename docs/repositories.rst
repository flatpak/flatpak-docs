...
Flatpak repositories are the primary mechanism for publishing applications, so
that they can be installed by users.
...
:doc:`flatpak-builder` covers the most common method for adding applications to
repositories.
...
`Flathub <http://flathub.org>`_, the primary publishing and hosting service for
...
As can be seen, the file includes the ID of the application and the location of
the repository that contains it, as well a link to information about the
...
installed, or the user will be prompted to install it. This will also happen if
the necessary runtime isn't present.
...
manually run ``flatpak update`` to check for and install new versions of any
